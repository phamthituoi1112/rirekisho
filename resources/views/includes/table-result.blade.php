...
<tr class="data" id='CV{{ $CV->id }}'>
    ...
    <td></td>
    @can('Visitor')
    <td class='bookmark' id='tag-bookmark'>
        @if ($CV->Bookmarks->contains(Auth::user()->id))
        <span class="glyphicon glyphicon-bookmark" style="color: darkorange; font-size: 50px"></span>
            <input id="id" type='hidden' value="{{ $CV->id }}">
        @endif
    </td>
    @endcan
    @can('Admin')
